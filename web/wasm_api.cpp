...
//
// This copy adds emu_set_sysroot: the VOICEVOX guest is a dynamically linked
// glibc program that opens its runtime, its dictionary and its model by
// absolute path, so it needs a filesystem root, and in a browser that root is a
// directory the page has filled in through emscripten's FS.
#include <cstdint>
#include <cstdlib>
#include <string>
#include <vector>
...
// Points the guest's `/` at a directory in the emscripten filesystem.  Pass an
// empty string to go back to having no root, which is what a static PE guest
// wants.
